...
ViewHelper.Actions = {
  ...
  startGame: function () {
    ...
    $('.menu').hide();
    $('.game-over').hide();
    playersHand.set(currentGame.currentPlayer().hand);
    opponentsHand.set(currentGame.opponent().hand);
  ...
  playCard: function ($card, $opponentsCard) {
    ...
    setTimeout(function () {
      ...
      currentGame.removePlayerCard($card);
      currentGame.removeOpponentCard($opponentsCard);
      currentGame.save();
